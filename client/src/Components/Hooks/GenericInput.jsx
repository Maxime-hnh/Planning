    ...
    return (
        <>
            ...
            <div className='mt-2.5'>
                {props.name !== 'contract.note' ?
                    <input
                        className={`bg-white block w-full rounded-md border-0 px-3.5 py-2 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset ${errorClass} sm:text-sm sm:leading-6`}
                        {...field}
                        {...props}
                    >
                    </input> :
